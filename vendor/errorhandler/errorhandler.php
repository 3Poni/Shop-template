...
class ErrorHandler
{
    ...
    public function register()
    {
        ...
        // регистрируем свою функцию, выполняющуюся перед завершением скрипта
    ...
    public function showError($errno, $errstr, $file, $line, $status = 500)
    {
        ...
        if (ERROR_HANDLER) {
            echo $message = '<br><b>' . self::getErrorName($errno) . "</b><hr>" . $errstr . '<hr> file: ' . $file . '<hr> line: ' . $line . '<hr><br>';
        } else {
            echo $status . ' Server ERROR';
            $message = '**start**' . PHP_EOL . self::getErrorName($errno) . $errstr . ' file: ' . $file . ' line: ' . $line . PHP_EOL . '**end**';
            error_log($message . PHP_EOL, 3, __DIR__ . '/logs/db_error.log');
        }
